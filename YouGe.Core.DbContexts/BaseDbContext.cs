...
namespace YouGe.Core.DbContexts
{
    public abstract class BaseDbContext : DbContext, IDbContextCore
    {
        ...
        public virtual int Add<T>(T entity, bool useTran) where T : class
        {

            base.Add(entity);

            return useTran ? 1 : SaveChanges();
        }
        ...
        public virtual async Task<int> AddAsync<T>(T entity, bool useTran) where T : class
        {

            await base.AddAsync(entity);
            return useTran ? 1 : await SaveChangesAsync();
        }

        public virtual int AddRange<T>(ICollection<T> entities, bool useTran) where T : class
        {
            base.AddRange(entities);
            return useTran ? entities.Count : SaveChanges();
        }

        public virtual async Task<int> AddRangeAsync<T>(ICollection<T> entities, bool useTran) where T : class
        {
            await base.AddRangeAsync(entities);
            return useTran ? entities.Count : await SaveChangesAsync();
        }
        ...
        public virtual int Delete<T, TKey>(TKey key, bool useTran) where T : BaseModel<TKey>
        {
        ...
        /// <returns></returns>
        public virtual int Edit<T>(T entity, bool useTran) where T : class
        {
        ...
        /// <returns></returns>
        public virtual int EditRange<T>(ICollection<T> entities, bool useTran) where T : class
        {
            foreach (T ta in entities)
            {
                base.Entry(ta).State = EntityState.Modified;
            }
        ...
        /// <returns></returns>
        public virtual int Update<T>(T model, bool useTran = false, params string[] updateColumns) where T : class
        {
        ...
        public virtual int Update<T>(Expression<Func<T, bool>> @where, Expression<Func<T, T>> updateFactory, bool useTran) where T : class
        {
            //to do Z.EntityFramework.Plus 事务如何处理？
            return GetDbSet<T>().Where(where).Update(updateFactory);
        }

        public virtual async Task<int> UpdateAsync<T>(Expression<Func<T, bool>> @where, Expression<Func<T, T>> updateFactory, bool useTran) where T : class
        {
            //to do Z.EntityFramework.Plus 事务如何处理？
        ...
        /// <returns></returns>
        public virtual int Delete<T>(Expression<Func<T, bool>> @where, bool useTran) where T : class
        {
            //to do Z.EntityFramework.Plus 事务如何处理？
            return GetDbSet<T>().Where(@where).Delete();
        }

        public virtual async Task<int> DeleteAsync<T>(Expression<Func<T, bool>> @where, bool useTran) where T : class
        {
            //to do Z.EntityFramework.Plus 事务如何处理？
        ...
        {
            return EF.CompileQuery((DbContext context, TKey tid) => context.Set<T>().Find(tid))(this, id);
        }
        public Task<T> GetByCompileQueryAsync<T, TKey>(TKey id) where T : BaseModel<TKey>
        {
            return EF.CompileAsyncQuery((DbContext context, TKey tid) => context.Set<T>().Find(tid))(this, id);
        }
        ...
        /// <param name="parms"></param>
        /// <returns></returns>
        public Task<List<T>> SelectBySqlAsync<T>(string sql, params object[] parms) where T : class, new()
        {
            return Database.SqlQueryAsync<T>(sql, parms);
        }
    }
}